...
import org.opengis.cite.osxeo.ETSAssert;
import org.opengis.cite.osxeo.ErrorMessage;
import org.opengis.cite.osxeo.ErrorMessageKeys;
import org.opengis.cite.osxeo.SuiteAttribute;
...
public class RelaxNGTest {
    ...
    private static final String ENTITY_CATALOG = "/org/opengis/cite/osxeo/schema-catalog.xml";

    /**
     * Obtains the test subject from the ISuite context and creates a
     * catalog-based resource resolver. The suite attribute
     * {@link org.opengis.cite.osxeo.SuiteAttribute#TEST_SUBJECT} should evaluate
    ...
    public void validateXMLfromRNC() throws SAXException, IOException {
        ...
        URL schemaRef = getClass().getResource(
                this.rncFile );
                //"/org/opengis/cite/osxeo/schemas/osdd.rnc");
...
/*
        URL schRef = this.getClass().getResource(
                "/org/opengis/cite/osxeo/tests/result-osd.rnc.sch");
        ETSAssert
...
*/
     ...
     * conformance classes defined in OGC 13-026
     *
     */
     /*
    @Test
    public void validateAdditionalConformance() throws SAXException, IOException {
        ...
        validateRNCandSCH( "/org/opengis/cite/osxeo/tests/result-osd.rnc");
        URL schemaRef = getClass().getResource(
                "/org/opengis/cite/osxeo/tests/result-osd.rnc");
        RelaxNGValidator validator = new RelaxNGValidator(schemaRef,
                this.resolver);
        Source xmlSource = (null != testSubject) ? new DOMSource(
                this.testSubject) : null;
...
*/
    ...
    public void checkSchematronRules() throws SAXException, IOException {
        URL schRef = this.getClass().getResource(
                "/org/opengis/cite/osxeo/sch/result-atom1.0.sch");
        ETSAssert
                .assertSchematronValid(schRef, new DOMSource(this.testSubject));
    }
*/
